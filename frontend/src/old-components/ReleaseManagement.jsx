    ...
    return (
        <div>
            ...
            {showCreateForm && (
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 mb-6">
                    <div className="px-6 py-4 border-b border-slate-100">
                        <h3 className="text-lg font-semibold text-slate-800">Create New Release</h3>
                    </div>
                    <div className="p-6">
                        <form onSubmit={handleCreateRelease}>
                            ...
                            <div className="mb-6">
                                <label className="block text-sm font-medium text-slate-700 mb-2">Release Description/Roadmap</label>
                                <textarea
            ...
            {canManageReleases && releases.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 mb-6">
                    <div className="px-6 py-4 border-b border-slate-100">
                        <h3 className="text-lg font-semibold text-slate-800">Upload to Release</h3>
                    </div>
                    <div className="p-6">
                        <form onSubmit={handleUpload}>
                            <div className="mb-4">
                                <label className="block text-sm font-medium text-slate-700 mb-2">Select Release *</label>
                                <select
                            ...
                            <div className="mb-6">
                                <label className="block text-sm font-medium text-slate-700 mb-2">Upload ZIP File *</label>
                                <input
                                    id="file-input"
                                    type="file"
                                    accept=".zip"
                            ...
                            {uploadFile && (
                                <div className="p-3 bg-blue-50 border border-blue-100 rounded-lg mb-4">
                                    <div className="font-medium text-blue-900 mb-1">Selected File:</div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200">
                <div className="px-6 py-4 border-b border-slate-100">
                    <h3 className="text-lg font-semibold text-slate-800">All Releases ({releases.length})</h3>
                </div>
                <div className="p-6">
                    {releases.length === 0 ? (
                        <div className="text-center py-16 text-slate-500 flex flex-col items-center">
                            ...
                            <h3 className="text-lg font-medium text-slate-700 mb-2">No Releases Found</h3>
                        ...
                        <div className="grid grid-cols-1 gap-6">
                            {releases.map((release) => (
                                <div key={release.id} className="relative border border-slate-200 rounded-xl p-6 hover:shadow-md transition-shadow group bg-slate-50/30">
                                        ...
                                        <div className="flex gap-2 items-center">
                                            ...
                                            {canManageReleases && (
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    className={release.isLocked ? 'text-amber-600 hover:text-amber-700' : 'text-slate-600'}
                                    ...
                                    <div className="bg-white border border-slate-200 rounded-lg p-3 text-sm text-slate-600 mb-4">
                                        <div className="grid grid-cols-3 gap-2">
                                            <div><span className="text-slate-400 font-normal">Release ID:</span> <span className="font-mono text-xs">{release.id}</span></div>
                                            <div><span className="text-slate-400 font-normal">Versions:</span> {release.versions.length}</div>
                                            {release.versions.length > 0 && (
                                                <div><span className="text-slate-400 font-normal">Latest:</span> v{release.versions[0].version}</div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {release.versions.length > 0 && (
                                        <div className="space-y-2">
                                            <h5 className="text-sm font-semibold text-slate-800 mb-2">Versions History</h5>
                                            {release.versions.map((version) => (
                                                <div key={version.id} className="flex justify-between items-center p-3 bg-white border border-slate-100 rounded-lg hover:border-emerald-200 transition-colors">
                                                    <div className="flex items-center gap-3">
                                                        <span className="font-mono text-sm font-medium text-slate-700">v{version.version}</span>
