...
/**
 * Creates a roadmap and its associated items within a transaction.
 * Note: projectId and roadmap.id (if any) are already numbers due to middleware.
 */
export const createRoadmapWithItems = async (tx, projectId, roadmap) => {
    ...
    const newRoadmap = await tx.roadmap.create({
        data: {
            projectId, // This is the raw integer ID
            title: roadmap.title,
    ...
    if (items && items.length > 0) {
        await tx.roadmapItem.createMany({
            data: items.map((item) => ({
                roadmapId: newRoadmap.id, // The ID generated by the DB
                title: item.title,
