...
export const listReleasesService = async (projectId, user) => {
    ...
    const release = await prisma.release.findMany({
        where: { projectId },
        include: {
            creator: {
...
export const lockReleaseService = async (releaseId, locked, user) => {
    ...
    const hasPermission =
        role === "admin" ||
        (role === "manager" && release.project.assignedManagerId === Number(decryptUser));
...
export const uploadReleaseVersionService = async (
    ...
    const markerScript = `<script>window.markerConfig = {project: '68b6da8e7a78dd9ff9cff850',
source: 'snippet'
};
